...
# BEFORE graph with (5,7) added
G_before = nx.Graph()
G_before.add_nodes_from(range(10))
G_before.add_edges_from(base_edges + [(5, 7)])

# AFTER graph with (5,7) removed, (5,8) added
G_after = nx.Graph()
G_after.add_nodes_from(range(10))
G_after.add_edges_from(base_edges + [(5, 8)])
...
for u, v in G_before.edges():
    style = 'solid'
    style = 'dotted' if (u, v) == (5, 7) or (v, u) == (5, 7) else 'solid'
    color = 'purple' if (u, v) == (5, 7) or (v, u) == (5, 7) else 'black'
    color = 'black'
    nx.draw_networkx_edges(G_before, pos, edgelist=[(u, v)], style=style,
                            edge_color=color, width=2, ax=axs[0])

# ======= MANUAL CONTROL OVER LABEL POSITION HERE =======
manual_x = 0.1   # <<< adjust this until you are satisfied
manual_y = -1.4   # <<< adjust this until you are satisfied
manual_angle = 60 # <<< adjust if you want a different rotation

axs[0].text(manual_x, manual_y, r"$\Pi_C - \Pi_D$", fontsize=28,
            ha='center', va='center', rotation=manual_angle, rotation_mode='anchor')
# ========================================================
...
# Title
plt.suptitle("Type B", fontsize=36)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig("TypeB", dpi=60)
plt.show()
